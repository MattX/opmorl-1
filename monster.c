...
void p_fight(int x, int y)
{
	...
	if(mon == NULL) return;
	mon->hp -= rodney.weapon->attack;
	if (rodney.weapon->class == C_BOW)
		rodney.arrows--;
	else if (rodney.weapon->class == C_WAND) {
		rodney.weapon->shots_left--;
		if (rodney.weapon->shots_left == 0) {
			rm_object(x, y); //this is not the appropriated func but I see no other one fitting.
		}
	}
